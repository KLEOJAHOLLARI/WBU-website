...
const ProfessorAnnouncements = () => {
  ...
  const { data: courses = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("courses").select("id, name, code").eq("professor_id", user!.id).order("name");
      return data || [];
    },
    enabled: !!user,
  });
  ...
  const { data: announcements = [], isLoading } = useQuery({
    queryKey: ["professor-announcements", user?.id],
    queryFn: async () => {
      const { data, error } = await supabase.from("announcements").select("*").eq("author_id", user!.id).order("created_at", { ascending: false });
  ...
  const saveMutation = useMutation({
    mutationFn: async (form: any) => {
      const payload = {
        ...
        author_id: user!.id,
        author_name: profile?.full_name || user!.email || "Professor",
      };
      if (form.id) {
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["professor-announcements"] });
      setShowForm(false);
      setEditing(null);
    ...
    <ProfessorLayout>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Post announcements for your courses</p>
        </div>
